...
const i = $e.defineComponent({ __name: "index", __ssrInlineRender: true, setup: (i2) => (s({ title: "\u7559\u8A00\u677F | copyman blog", description: "\u8BBF\u5BA2\u7559\u8A00\u4E0E\u4E92\u52A8" }), (r2, i3, l2, d) => {
  const a = p;
  i3(`<div${kr.ssrRenderAttrs($e.mergeProps({ class: "flex-1 w-full h-full flex flex-col gap-6 p-3 bg-white" }, d))}><!--[-->`), kr.ssrRenderList($e.unref(n), (e2) => {
    i3(kr.ssrRenderComponent(a, { key: e2.id, to: `/message/message_${e2.id}`, class: "flex w-full gap-4 border-b last:border-none border-[#f1f1f1] pb-10 group cursor-pointer text-left" }, { default: $e.withCtx((t2, r3, i4, l3) => {
      ...
      r3(`<div class="w-12 h-12 md:w-14 md:h-14 shrink-0 rounded-full overflow-hidden bg-[#f5f5f5] ring-1 ring-[#eee]"${l3}><img class="w-full h-full object-cover"${kr.ssrRenderAttr("src", e2.avatar)}${kr.ssrRenderAttr("alt", e2.userName)}${l3}></div><div class="flex flex-col gap-2 min-w-0 flex-1"${l3}><div class="flex flex-wrap items-baseline justify-between gap-2 text-[14px] text-[#999] italic font-thin"${l3}><span class="text-[#333] not-italic font-bold group-hover:text-primary transition-colors"${l3}>${kr.ssrInterpolate(e2.userName)}</span><span${l3}>${kr.ssrInterpolate($e.unref(i$1)(e2.createdAt))}</span></div><div class="text-[14px] text-[#666] group-hover:underline decoration-solid decoration-[#999] line-clamp-3 leading-relaxed"${l3}>${kr.ssrInterpolate(e2.content)}</div></div>`);
    }), _: 2 }, l2));
  }), i3("<!--]--></div>");
}) }), l = i.setup;
i.setup = (e2, o2) => {
...
export { i as default };
//# sourceMappingURL=index-DipyY4QA.mjs.map
